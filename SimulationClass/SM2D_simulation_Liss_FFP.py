...
from recon_final.BaseClass.Simulation2D_Base import *
...
class SM_simulation_2D_Liss_FFP_Measurement(SM2D_SimulationBase):
    ...
    def _get_AuxSignal(self):
        ...
        system_matirx = np.zeros(((self._sample_num // 2 + 1) * 2,self._X_num * self._Y_num),dtype = complex)
        ...
        for i in tqdm(range(self._X_num * self._Y_num),desc=self._title_auxsignal):
            ...
            for k in range(self._sample_num):
                ...
                voltage[0,k] = Langevin_derivative * coeff[0] * self._delta_volume * self._delta_concentration * total_mod
                voltage[1,k] = Langevin_derivative * coeff[1] * self._delta_volume * self._delta_concentration * total_mod
